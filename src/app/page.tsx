                ...
                {showLogs && (
                  <div style={{ marginTop: 8, background: '#fff', border: '1px solid #eee', borderRadius: 8, padding: 12, fontSize: 12, color: '#666' }}>
                    {logs.length === 0 ? <span>로그 없음</span> : logs.map((log, i) => (
                      <div key={i} style={{ padding: '4px 0', borderBottom: '1px solid #f5f5f5', display: 'flex', alignItems: 'baseline', gap: 6, flexWrap: 'wrap' }}>
                        ...
                        <span style={{ fontSize: 12, color: '#666' }}>{log.message}</span>
                        <span style={{ fontSize: 11, color: '#888', background: '#f5f5f5', padding: '1px 6px', borderRadius: 3, marginLeft: 14 }}>{toKST(log.created_at)}</span>
                      </div>
                    ))}
                  </div>
                )}
